...
def build_header(  # pylint: disable=R0914
    ...
    draw.text(
        (uh_left, uh_top), user_handle, fill=TEXT_SECONDARY, font=USER_HANDLE_FONT
    )
...
async def get_header(  # pylint: disable = R0914
    ...
    # fallback_file_id = (
    #     user_data.fallback_photo.file_id if user_data.fallback_photo else None
    # )
    # fallback_unique_id = (
    ...
    if (
        ...
        and user_data.photo_file_unique_id
        == (photo_file_unique_id or fallback_unique_id)
    ):
        ...
        try:
            ...
            logger.debug(
                "Opening existing header for user %s failed. Building new header.",
                user.id,
            )
    else:
...
def build_body(
    text: str, text_direction: str = LTR, event: Event = None
) -> Image.Image:
    """
