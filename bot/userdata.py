#!/usr/bin/env python3
"""A custom user data class."""
from asyncio import Event, Task
from typing import Callable, Dict, Optional, Tuple, Union
from collections import deque

from telegram import PhotoSize, User
from telegram.ext import CallbackContext, ExtBot

from bot.constants import LTR


class UserData:  # pylint: disable=R0902
    """Custom class to store information relevant for each user.

    Args:
        user_id: The users id.
        username: The users username
        first_name: The users first name.
        full_name: The users full name.
        tweet: The users tweet.
        photo_file_unique_id: The users current photos unique file id.
        fallback_photo: The users fallback photo
        sticker_file_ids: Mapping file unique id → file id of stickers sent by the user for later
            use.
        temp_file_ids: Temporarily saved file ids of stickers generated by the user.
        store_stickers: Whether or not this users wants to store stickers. Defaults to :obj:`True`.
        tzinfo: Users timezone. Defaults to UTC.
        text_direction: Users preferred text direction. Defaults to left-to-right.

    Attributes:
        user_id: The users id.
        username: The users username
        first_name: The users first name.
        full_name: The users full name.
        photo_file_unique_id: The users current photos unique file id.
        fallback_photo: The users fallback photo
        sticker_file_ids: Mapping file unique id → file id of stickers sent by the user for later
            use.
        temp_file_ids: Temporarily saved file ids of stickers generated by the user.
        store_stickers: Whether or not this users wants to store stickers. Defaults to :obj:`True`.
        inline_query_task: Optional. A :class:`asyncio.Task` which answers the current inline
            query.
        inline_query_event: Optional. An :class:`tasking.Event`. Setting this event will cause
            :attr:`inline_query_task` to terminate.
        tzinfo: Users timezone.
        text_direction: Users preferred text direction.
    """

    __slots__ = (
        "user_id",
        "username",
        "first_name",
        "full_name",
        "access_token",
        "access_token_secret",
        "tweet",
        "tweet_id",
        "tmp_message_id",
        "photo_file_unique_id",
        "fallback_photo",
        "sticker_file_ids",
        "temp_file_ids",
        "store_stickers",
        "inline_query_task",
        "inline_query_event",
        "tzinfo",
        "text_direction",
        "timeline_tokens",
        "timeline_tweets",
        "jwt",
        "address",
    )

    def __init__(  # pylint: disable=R0913
        self,
        user_id: Optional[int] = None,
        username: Optional[str] = None,
        first_name: Optional[str] = None,
        full_name: Optional[str] = None,
        access_token: Optional[str] = None,
        access_token_secret: Optional[str] = None,
        tweet: Optional[str] = None,
        tweet_id: Optional[str] = None,
        tmp_message_id: Optional[Tuple[int]] = None,
        photo_file_unique_id: Optional[str] = None,
        fallback_photo: Optional[PhotoSize] = None,
        sticker_file_ids: Dict[str, str] = None,
        temp_file_ids: Dict[str, Tuple[str, str]] = None,
        store_stickers: Optional[bool] = True,
        tzinfo: str = "UTC",
        text_direction: str = LTR,
        timeline_tokens: Optional[int] = None,
        timeline_tweets: Optional[deque] = None,
        jwt: Optional[str] = None,
        address: Optional[str] = None,
    ):
        self.user_id = user_id
        self.username = username
        self.first_name = first_name
        self.full_name = full_name
        self.access_token = access_token
        self.access_token_secret = access_token_secret
        self.tweet = tweet
        self.tweet_id = tweet_id
        self.tmp_message_id = tmp_message_id
        self.photo_file_unique_id = photo_file_unique_id
        self.fallback_photo = fallback_photo
        self.store_stickers = store_stickers
        self.tzinfo = tzinfo
        self.text_direction = text_direction

        self.sticker_file_ids: Dict[str, str] = sticker_file_ids or {}
        self.temp_file_ids: Dict[str, Tuple[str, str]] = temp_file_ids or {}

        self.inline_query_task: Optional[Task] = None
        self.inline_query_event: Optional[Event] = None
        self.timeline_tokens = timeline_tokens
        self.timeline_tweets = timeline_tweets
        self.jwt = jwt
        self.address = address

    def update_user_info(self, user: User, photo_file_unique_id: Optional[str]) -> None:
        """Updates the stored info about the user with the fresh users instance.

        Args:
            user: The User instance.
            photo_file_unique_id: The up to date unique file id.
        """
        self.user_id = user.id
        self.first_name = user.first_name
        self.full_name = user.full_name
        self.username = user.username
        self.photo_file_unique_id = photo_file_unique_id

    def update_jwt(self, jwt):
        self.jwt = jwt


    def update_address(self, address):
        self.address = address

    def update_timeline_tweets(self, timeline_tweets: deque):
        """Updates stored timeline tweets information.

        Args:
            users: The User instance.
            timeline_tweets: The content of the timeline tweets to be updated.
        """
        self.timeline_tweets = timeline_tweets

    def update_tweet(self, user: User, tweet: str):
        """Updates stored tweet information.

        Args:
            users: The User instance.
            tweet: The content of the tweet to be updated.
        """
        self.tweet = tweet

    def update_tweet_id(self, user: User, tweet_id: str):
        """Updates stored tweet information.

        Args:
            users: The User instance.
            tweet_id: The id of the tweet to be updated.
        """
        self.tweet_id = tweet_id

    def update_timeline_tokens(self, timeline_tokens: int):
        """Updates stored timeline tokens information.

        Args:
            users: The User instance.
            timeline_tokens: The content of the timeline tokens to be updated.
        """
        self.timeline_tokens = timeline_tokens

    def update_tmp_message_id(self, user: User, tmp_message_id: Tuple[int, int]):
        """Updates stored temp message id information.

        Args:
            users: The User instance.
            temp_message_id: The id of the temp message to be updated.
        """
        self.tmp_message_id = tmp_message_id

    def update_access_token(self, access_token: str):
        """Updates stored access token information.

        Args:
            users: The User instance.
            access_token: The content of the access token to be updated.
        """
        self.access_token = access_token

    def update_access_token_secret(self, access_token_secret: str):
        """Updates stored access token secret information.

        Args:
            users: The User instance.
            access_token_secret: The content of the access token secret to be updated.
        """
        self.access_token_secret = access_token_secret

    def update_fallback_photo(self, fallback_photo: Optional[PhotoSize]) -> None:
        """Updates fallback photo for this user.

        Args:
            fallback_photo: The new fallback photo.
        """
        self.fallback_photo = fallback_photo

    def __getattr__(self, item: str) -> Union[None, str]:
        # Only called when `self.item` raises AttributeError
        # But pickle calls it for some magic methods, too …
        if item == "tzinfo":
            return "UTC"
        if item == "text_direction":
            return LTR
        if item.startswith("__") and item.endswith("__"):
            return super().__getattr__(item)  # type: ignore[misc]  # pylint: disable=E1101
        return None

    _INIT_ARGS = Tuple[
        Optional[int],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[Tuple[int]],
        Optional[PhotoSize],
        Dict[str, str],
        Dict[str, Tuple[str, str]],
        Optional[bool],
        str,
        str,
        Optional[int],
        Optional[deque],
        Optional[str],
        Optional[str],
    ]
    _CALLABLE_ARGS = [
        Optional[int],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[str],
        Optional[Tuple[int]],
        Optional[PhotoSize],
        Dict[str, str],
        Dict[str, Tuple[str, str]],
        Optional[bool],
        str,
        str,
        Optional[int],
        Optional[deque],
        Optional[str],
        Optional[str],
    ]

    # We override __reduce__ to amend for the unpickable task & event
    def __reduce__(
        self,
    ) -> Tuple[Callable[_CALLABLE_ARGS, "UserData"], _INIT_ARGS]:  # type: ignore[valid-type]
        return self.__class__, (
            self.user_id,
            self.username,
            self.first_name,
            self.full_name,
            self.access_token,
            self.access_token_secret,
            self.tweet,
            self.tweet_id,
            self.tmp_message_id,
            self.photo_file_unique_id,
            self.fallback_photo,
            self.sticker_file_ids,
            self.temp_file_ids,
            self.store_stickers,
            self.tzinfo,
            self.text_direction,
            self.timeline_tokens,
            self.timeline_tweets,
            self.jwt,
            self.address,
        )


CCT = CallbackContext[ExtBot, UserData, dict, dict]  # type: ignore[misc]
"""Type alias for :class:`telegram.ext.CallbackContext` with :class:`UserData` as user data."""
