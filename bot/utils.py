...
from bot.twitter_sticker import build_sticker
...
async def conversation_fallback(update: Update, context: CCT) -> int:
    ...
    """
    await cast(User, update.effective_user).send_message(
        "Ending Conversation. Aborting operation."
    )
    await remove_reply_markup(context)
...
async def cancel(update: Update, context: CCT):
    """
